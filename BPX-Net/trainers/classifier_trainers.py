...
class ClassifierTrainer(BaseTrainer):
    ...
    def start_train(self, train_dataset, save_model_dir, val_dataset=None, max_ck_files_to_save=2, pretrained_file=None):
        '''        
        ...
        train_dataloader = DataLoader(train_dataset, batch_size=self.batch_size, num_workers=2, persistent_workers=True, shuffle=True)
        ...
        df_summary = pd.DataFrame(columns = ['time', 'step', 'loss', 'accuracy'])
        ...
        for epoch in range(1, self.epochs+1):
            ...
            loss = self.__train_epoch(train_dataloader, 
                                      optimizer,
                                      scheduler,
                                      grad_scaler,
                                      loss_func)
            ...
            print ('\nloss: %.3f; Lr: %.6f; Used time (s): %.4f; accuracy: %.10f' %
                    (loss, current_lr, t2-t1, acc))
            ...
            list = [current_time, step, train_loss, accuracy_str]
    ...
    def __train_epoch(self, train_dataloader, optimizer, scheduler, grad_scaler, loss_func):
        losses = []
        self.model.train()
        for step, (xs, ys) in enumerate(train_dataloader):
            ...
            with torch.cuda.amp.autocast(enabled=False):
                coeffs, out_scores = self.model(xs)#coeffs:the learned weigths as feature importance
    ...
    def __calc_accuracy(self, eval_dataloader):
        ...
        for xs, ys in eval_dataloader:
            ...
            ys = torch.tensor(ys, dtype=torch.long).to(self.device)
            with torch.no_grad():
                _, out_scores = self.model(xs)
            
            predicted = torch.argmax(out_scores, dim=1)
            acc = torch.sum(predicted==ys)/xs.shape[0]
